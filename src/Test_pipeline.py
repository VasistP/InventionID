...
class InventionExtractionAgent:
    ...
    def __init__(self, bedrock_client, model_id, max_iterations=6, log_dir="logs", debug=False):
    ...
    def _extract_invention(self, text, chunks):
        ...
        self._log("retrieval_extract", {
                "top_k": len(top),
                "chunk_ids": [c["id"] for c in top],
                "chunk_previews": [{"id": c["id"], "preview": c["text"][:120]} for c in top],
            })
...
- Use ONLY chunk IDs that appear in CONTEXT CHUNKS above. Do not invent IDs."""
        ...
        payload = {"prompt_preview": prompt[:500], "response_preview": response[:300]}
        if self.debug:
            payload["response_full"] = response
        self._log("extract_invention", payload)
...
- Return JSON only, no markdown fences, no other text."""
        ...
        payload = {
            "invention_name": invention.get("invention_name"),
            "schema_ok": invention.get("_schema_ok", None),
            "response_preview": response[:300],
        }
        if self.debug:
            payload["response_full"] = response
        self._log("validate_invention", payload)
    ...
    def _refine_invention(self, invention, suggestions, document_text, chunks):
        ...
        self._log("retrieval_refine", {
            "top_k": len(top),
            "chunk_ids": [c["id"] for c in top],
            "chunk_previews": [{"id": c["id"], "preview": c["text"][:120]} for c in top],
            "suggestions_preview": str(suggestions)[:200],
        })
        context = "\n\n".join([f"[CHUNK {c['id']}] {c['text']}" for c in top])
...
- All citations must reference only the chunk IDs shown in CONTEXT CHUNKS."""
        ...
        response = self._call_llm(prompt, max_tokens=1500)
        payload = {"suggestions_preview": str(suggestions)[:200], "response_preview": response[:300]}
        if self.debug:
            payload["response_full"] = response
        self._log("refine_invention", payload)
...
# Create a unique run id every time
        self.run_id = datetime.now().strftime("%Y%m%d_%H%M%S") + "_" + uuid.uuid4().hex[:8]

        # Run meta (helps compare runs)
        self.run_meta = {
            "run_id": self.run_id,
            "document_length": len(document_text),
            "model_id_agent": self.model_id,
            "confidence_threshold": self.CONFIDENCE_THRESHOLD,
            "max_iterations": self.max_iterations,
        }

        self._log("run_start", self.run_meta)

        chunks = get_chunks(document_text)

        # If your chunker exposes params, log them here (placeholders)
        self.chunking_meta = {
            "num_chunks": len(chunks),
            "chunk_size": None,
            "chunk_overlap": None
        }
        self._log("chunking", self.chunking_meta)
        ...
        # 1) Add *_refs keys next to each field (from citations)
        refs = evidence.get("field_refs", {})
        if isinstance(refs, dict):
            for field, ids in refs.items():
                final_invention[f"{field}_refs"] = ids

        # 2) Add chunk_snippets map (one snippet per chunk id)
        final_invention["chunk_snippets"] = evidence.get("chunk_snippets", {})

        # 3) Keep evidence small (just meta)
        final_invention["evidence"] = {
            "has_citations": evidence.get("has_citations"),
            "missing_chunk_ids": evidence.get("missing_chunk_ids", {}),
        }

        # 4) Remove the old big citations dict (since *_refs replaces it)
        final_invention.pop("citations", None)
        self._log("evidence_summary", {
            "has_citations": evidence.get("has_citations"),
            # "fields_with_citations": evidence.get("fields_with_citations"),
            # "fields_total_cited": evidence.get("fields_total_cited"),
            "missing_chunk_ids": evidence.get("missing_chunk_ids"),
        })
        ...
        print("Chunk snippets keys:", list(final_invention.get("chunk_snippets", {}).keys())[:10])
