...
import os
import copy
import tools.arxiv_search as arx
...
def _run_pipeline_once(bucket, pdf_key, progress_callback=None):
    ...
    if not queries:
        ...
        if not patents and not papers:
            ...
            _emit(5, "Analyzing prior art", detail=f"{len(detailed)} patents, {len(detailed_arxiv)} papers")
            invention_desc = (
            ...
            try:
                analysis_arxiv = arx.analyze_papers(invention_desc, detailed_arxiv, call_bedrock_json)
            except JsonParseExhaustedError:
                raise
            except Exception as e:
    ...
    # Add arxiv paper analysis to report
    report["arxiv_papers_found"] = len(detailed_arxiv)
    report["arxiv_papers_analyzed"] = len(analysis_arxiv)
    report["arxiv_papers"] = analysis_arxiv
