...
class ArxivPaperSearch:
    def __init__(self, max_results=10, max_workers=4):
        self.client = arxiv.Client()
        self.max_results = max_results
        self.max_workers = max_workers

    def _search_one(self, query_input):
        search = arxiv.Search(
            query=query_input,
    ...
    def paper_search_parallel(self, queries):
        """
        ...
        # arXiv can be rate-limited; don't go crazy with workers
        max_workers = min(self.max_workers, len(queries)) or 1

        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            future_to_query = {ex.submit(self._search_one, q): q for q in queries}
