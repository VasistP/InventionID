...
def parallel_search_queries(
    ...
    Returns a list of (query, results_list, total_count) tuples ordered to
    match the original `queries` order. total_count is Google Patents' full
    hit count (used for calibration feedback to the user).
    """
    queries = [q for q in queries if q and q.strip()]
    if not queries:
        return []
    ...
    def run_query(q: str):
        try:
            print(f"  [SEARCH] {q}")
            results, total = patent_searcher.search(q, max_results=max_results_per_query)
            print(f"  [DONE] {q} -> {len(results)} fetched, {total:,} total")
            return q, results, total
        except Exception as e:
            print(f"  [ERROR] query={q!r}: {e}")
            return q, [], 0

    results_by_query: List[Tuple[str, List[Dict[str, Any]], int]] = []
    ...
    with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
        ...
        for future in as_completed(future_to_query):
            q = future_to_query[future]
            try:
                q_res, res, total = future.result()
            except Exception as e:
                print(f"  [FATAL] query={q!r} crashed: {e}")
                q_res, res, total = q, [], 0
            results_by_query.append((q_res, res, total))
